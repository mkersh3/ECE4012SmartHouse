...
// Do not touch this class whatsoever. If changes need to be made, go to emailInterface.cs.
namespace SmartHouseSD
{
    ...
    public class Pop3MailClient
    {
        ...
        public bool GetRawEmail(int MessageNo, out string EmailText)
        {
            ...
            EmailText = RawEmailSB.ToString();    // Email in full after receiving.
            //string CurDir = Directory.GetCurrentDirectory();  // Current Directory String for saving EmailText
            //File.WriteAllText(CurDir + "\\MessageText" + MessageNo + ".txt", EmailText);  // Write email to MessageTextX.txt
